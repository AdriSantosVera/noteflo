...
export default function NoteDetailScreen() {
  ...
  const handleReminder = () => {
    ...
    void (async () => {
      ...
      Alert.alert(
        ...
        `¿Cuándo quieres que te recordemos "${note?.title ?? 'esta nota'}"?`,
        [
          ...REMINDER_PRESETS.map((preset) => ({
            text: preset.label,
            onPress: () => {
              const triggerDate = buildReminderDate(preset.value);
              void scheduleReminder(note?.id ?? '', note?.title ?? 'Nota', triggerDate).then(
                (newId) => {
                  if (newId) {
                    setReminderId(newId);
  ...
  const confirmDelete = () => {
    if (Platform.OS === 'web' && typeof globalThis.confirm === 'function') {
      return Promise.resolve(globalThis.confirm('Esta acción borrará la entrada del store.'));
    }
        ...
        <LinearGradient colors={['rgba(17, 26, 46, 0.96)', 'rgba(9, 14, 24, 0.88)']} end={{ x: 1, y: 1 }} start={{ x: 0, y: 0 }} style={styles.card}>
          ...
          {note.location_name ? (
            <View style={styles.locationRow}>
              ...
              <Text numberOfLines={1} style={styles.locationText}>{note.location_name}</Text>
            </View>
          ) : null}
